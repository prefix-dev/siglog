...
pub fn read_snapshot(path: &Path) -> Option<(u64, HashMap<IndexKey, Vec<LogIndex>>)> {
    ...
    parse_snapshot(&data)
        .map_err(|e| {
            tracing::warn!("Ignoring invalid vindex snapshot {}: {}", path.display(), e);
        })
        .ok()
}
...
fn parse_snapshot(data: &[u8]) -> Result<(u64, HashMap<IndexKey, Vec<LogIndex>>)> {
    ...
    if key_count > (body.len() - 21) / 36 {
        return Err(Error::Internal(
            "snapshot key count exceeds available data".into(),
        ));
    }
    let mut pos = 21usize;
    ...
    for _ in 0..key_count {
        ...
        for i in 0..idx_count {
            ...
            if v >= tree_size
                || indices
                    .last()
                    .is_some_and(|last: &LogIndex| last.value() >= v)
            {
                return Err(Error::Internal(
                    "snapshot indices must be ordered and below tree size".into(),
                ));
            }
            indices.push(LogIndex::new(v));
        }
        pos += idx_count * 8;
        if index.insert(key, indices).is_some() {
            return Err(Error::Internal("duplicate snapshot key".into()));
        }
    }
