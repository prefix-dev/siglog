...
///
/// Each entry is serialized before writing. A failed write may leave a torn
/// record; startup recovery detects it and rebuilds from log storage if needed.
...
impl BatchedBinaryWalWriter {
    ...
    /// rewrites the full batch. Duplicate records fail the contiguous replay
    /// check and trigger a rebuild from log storage; CRC validation detects
    /// torn fragments on startup.
