...
impl VerifiableIndex {
    ...
    pub fn with_wal_and_batch_size(
        ...
        if wal_path.exists() {
            ...
            while let Some((idx, keys)) = reader.next_entry()? {
                ...
                if idx.value() != tree_size {
                    return Err(Error::Internal(format!(
                        "non-contiguous vindex WAL: expected {tree_size}, got {}; rebuild required",
                        idx.value()
                    )));
                }
                for key in keys {
    ...
    pub async fn rebuild_from_storage(
        ...
        let snapshot_file = snapshot::snapshot_path(wal_path);
        for path in [snapshot_file.as_path(), wal_path] {
        ...
        for bundle_idx in 0..=last_bundle {
            ...
            let expected_entries = (expected_tree_size - bundle_idx * ENTRY_BUNDLE_WIDTH)
                .min(ENTRY_BUNDLE_WIDTH) as usize;
            if bundle.entries.len() != expected_entries {
                return Err(Error::Internal(format!(
                    "entry bundle {bundle_idx} has {} entries, expected {expected_entries}",
                    bundle.entries.len()
                )));
            }
...
mod tests {
    ...
    async fn test_rebuild_from_storage() {
        ...
        storage
            .write_entry_bundle(
                TileIndex::new(0),
                PartialSize::new(3),
                &EntryBundle::with_entries(vec![]),
            )
            .await
            .unwrap();
        assert!(VerifiableIndex::rebuild_from_storage(
    ...
    #[test]
    fn snapshot_replay_rejects_gap_after_snapshot() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("index.wal");
        let map = Arc::new(JsonKeysMapFn::new("name"));
        let index = VerifiableIndex::with_wal(map.clone(), &path, 0).unwrap();
        index
            .index_entry(LogIndex::new(0), br#"{"name":"a"}"#)
            .unwrap();
        index.snapshot().unwrap();
        drop(index);
        let mut wal = WalWriter::open(&path).unwrap();
        wal.append(LogIndex::new(2), &[hash_key("b")]).unwrap();
        wal.flush().unwrap();
        drop(wal);
        assert!(VerifiableIndex::with_wal(map, &path, 3).is_err());
    }
