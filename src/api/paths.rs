...
mod tests {
    ...
    fn test_parse_tile_level() {
        assert!(parse_tile_level("+1").is_err());
        assert!(parse_tile_index("+01").is_err());
        assert!(parse_tile_index("001.p/+1").is_err());
        assert_eq!(partial_tile_size(u64::MAX, 0, u64::MAX), 0);
