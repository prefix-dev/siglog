...
This is a **repository-maintained regression suite**, not an official C2SP certification.
It exercises the [C2SP tlog-witness](https://c2sp.org/tlog-witness) protocol and
[tlog-cosignature](https://c2sp.org/tlog-cosignature) responses against a real process.

CI runs `uv run --locked pytest --entrypoint=../target/debug/witness -v`.
Every successful checkpoint response is independently verified using Python's
`cryptography` Ed25519 implementation: signer name, alg-0x04 key ID, 76-byte
encoding, timestamp, and signature over the exact checkpoint body. Negative
self-tests ensure the verifier rejects tampering and legacy plain signatures.
The suite also tests persistence across restart and concurrent conflicting
checkpoints, and fails rather than silently skipping missing case files.

Limits: the Merkle proofs are generated by our own Python helper; HTTP tests use
SQLite and do not simulate power loss or test every legal C2SP input. Rust tests
cover WAL recovery, atomic state updates (including PostgreSQL in CI), strict key
verification, and monitor validation. Passing this suite is useful regression
evidence, not a complete security audit or independent interoperability claim.
...
The suite generates its own matching log key pair. Do not pass `--log-config`:
a public verification key alone cannot sign test checkpoints.
